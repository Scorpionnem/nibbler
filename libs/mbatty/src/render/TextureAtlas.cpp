...
void	TextureAtlas::add_texture(const std::string& path, const std::vector<u8>& pixels, int width, int height, int channels, GLenum format)
{
	(void)format;
...
void        TextureAtlas::bind(u32 unit) const {_texture.bind(unit);}
bool        TextureAtlas::has(const std::string& path) const {return (_uvs.find(path) != _uvs.end());}

vec4f       TextureAtlas::uv(const std::string& path) const
{
...
const Texture&  TextureAtlas::texture() const {return (_texture);}
u32         TextureAtlas::size() const {return (_size);}

bool    TextureAtlas::_isSpotFree(const aabb2i& target_box) const
{
...
bool   TextureAtlas::_findSpot(const vec2i& size, vec2i& out_pos)
{
...
void    TextureAtlas::_insertTexture(const std::vector<u8>& rgba, int width, int height, const vec2i& pos)
{
...
std::vector<u8> TextureAtlas::_toRGBA(const std::vector<u8>& data, int width, int height, int channels)
{
